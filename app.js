 ...
 *
 * This file represents the main entry point of the Nebulactyl application.
...
if (cluster.isMaster) {
  const configuredWorkers = settings.cluster && Number(settings.cluster.workers);
  const numCPUs = configuredWorkers > 0 ? configuredWorkers : Math.max(1, Math.min(os.cpus().length, 4));
  console.log(chalk.gray('Starting workers on Nebulactyl'))
  ...
  app.use((req, res, next) => {
    res.setHeader("X-Powered-By", "Nebulactyl");
    next();
  });
